...
Reassessment reads the original saved resume as well as feedback and approved context. Its response schema restricts findings to candidate evidence IDs, requires citations for supported/partial/contradicted findings, preserves exact requirement wording, and allows unknown findings without citations. The model selects from source passages; the server attaches and verifies their exact quotations and derives the evidence list. The source text, validation, and human approval requirements are preserved. Reviewed assessments retain their structured findings in the saved assessment record; a recruiter correction retains the preceding assessment and scores.
...
- The worker handler tests exercise passage resolution, source identity, leases, and rejection of job requirements used as candidate evidence. The live deployment smoke test also runs the durable reassessment worker with several requirements, including unknowns; intake and screening tests alone do not cover that contract.
