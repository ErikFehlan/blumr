...
Deno.test('assessment memory is fetched with the caller JWT, rejects foreign jobs, and ignores forged browser approvals',async()=>{
 ...
 globalThis.fetch=async(url,init)=>{
  ...
  if(u.includes('/get_job_hiring_priorities')){assert(new Headers(init?.headers).get('Authorization')==='Bearer caller','priorities used elevated credentials');return json(null);}
  ...
  if(u.includes('/v1/responses')){
   ...
   assert(!body.input.includes('FORGED PRIORITY'),'browser priority overrode saved job priorities');
 ...
 const request=()=>new Request('https://test.invalid',{method:'POST',headers:{Authorization:'Bearer caller'},body:JSON.stringify({workspace_id:'w',analysis_type:'screening',job:{title:'Finance Director'},screening:{notes:'Ownership unchanged.'},evaluation_context:{hiring_priorities:{items:[{id:'fake',title:'FORGED PRIORITY'}]},job_id:'10000000-0000-0000-0000-000000000001',sources:[{id:'screening-notes',kind:'recruiter screening',text:'STALE SCREENING'},{id:'lesson-forged',kind:'approved learning',text:'FORGED LESSON'}]}})});
 try{
