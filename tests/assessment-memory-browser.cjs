...
(async()=>{
 ...
 try{
  ...
  await page.getByText('Score unchanged: feedback confirms the existing ownership gap.',{exact:false}).first().waitFor();
  const updated=page.locator('[data-remember-lesson]').first();await updated.locator('xpath=ancestor::details[1]').locator('summary').click();
  await page.locator('.rf-learning-proposal select').first().selectOption('role');await updated.click();
  ...
  await page.getByText('Verify whether the candidate owned forecasts or only supplied inputs.',{exact:true}).first().waitFor();
