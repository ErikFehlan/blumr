...
(async () => {
  ...
  try {
    ...
    await page.getByRole('link',{name:'Why blumr Works',exact:true}).click();
    await page.waitForURL(url=>url.origin===new URL(target).origin&&/^\/why-blumr-works(?:\.html)?$/.test(url.pathname));
    await page.locator('#an-why-title').waitFor({state:'visible'});
    assert.equal(await page.locator('#about, #rf-app, #authForm').count(),0,'Why blumr Works should be its own public page');
    ...
    await fs.writeFile('test-results/live/probe.json', JSON.stringify({url:target,status:'passed',checks:['HTTPS','homepage','Why blumr Works page and reload','About page and reload','application startup','sign-in form']},null,2));
