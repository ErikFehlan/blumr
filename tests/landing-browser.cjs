...
(async()=>{
 ...
 try{
  ...
  await page.reload();assert.ok((await page.locator('#an-about-title').boundingBox()).y<400,'About opens at the top on direct loads');
  ...
  for(const width of [820,390,320]){
   ...
   assert.ok((await page.locator('#an-about-title').boundingBox()).y<400,'About title is at the top at '+width);
  ...
  assert.deepEqual(errors,[]);console.log('Public homepage passed: standalone About page, direct loading, About account links, product preview, keyboard tabs, sign-in and signup entry points, unchanged recovery URLs and callback, private app hidden, and desktop/small-screen layout.');
 }catch(error){
